...
Set_Clipboard(str) {
	...
	ClipWait, 2, 1
	if (ErrorLevel) {
		trayMsg := StrReplace("Unable to put the following content in the clipboard: %message%.`nA clipboard manager may be creating conflict.", "%message%", str)
		TrayNotifications.Show("Failed to set the clipboard", trayMsg)
		return 1
	}
